...
const i18n: Record<Locale, {
  ...
  record: string; level1: string; level2: string; level3: string; start: string; noMove: string;
}> = {
  ko: { title: "도미노", you: "나", cpu: "AI", turn: "차례", reset: "새 판", draw: "가져오기", pass: "패스", boneyard: "더미", yourTiles: "내 패", left: "◀ 왼쪽", right: "오른쪽 ▶", pickEnd: "어느 쪽에 놓을까요?", youWin: "당신의 승리!", cpuWins: "AI 승리", blockDraw: "무승부 (막힘)", blocked: "막힘 — 남은 패 점수로 승부", thinking: "AI가 생각 중…", record: "전적", level1: "견습생", level2: "숙련가", level3: "명인", start: "게임 시작", noMove: "놓을 패가 없습니다." },
  en: { title: "Dominoes", you: "You", cpu: "AI", turn: "Turn", reset: "New Game", draw: "Draw", pass: "Pass", boneyard: "Boneyard", yourTiles: "Your tiles", left: "◀ Left", right: "Right ▶", pickEnd: "Which end?", youWin: "You win!", cpuWins: "AI wins", blockDraw: "Draw (blocked)", blocked: "Blocked — fewest pips wins", thinking: "AI is thinking…", record: "Record", level1: "Apprentice", level2: "Adept", level3: "Master", start: "Start Game", noMove: "No playable tile." },
  ja: { title: "ドミノ", you: "あなた", cpu: "AI", turn: "手番", reset: "新しいゲーム", draw: "引く", pass: "パス", boneyard: "山札", yourTiles: "手札", left: "◀ 左", right: "右 ▶", pickEnd: "どちらに置く？", youWin: "あなたの勝ち！", cpuWins: "AIの勝ち", blockDraw: "引き分け（詰み）", blocked: "詰み — 手札の点数で勝負", thinking: "AIが思考中…", record: "戦績", level1: "見習い", level2: "熟練者", level3: "名人", start: "ゲーム開始", noMove: "置ける牌がありません。" },
  zh: { title: "多米诺骨牌", you: "你", cpu: "AI", turn: "回合", reset: "新对局", draw: "摸牌", pass: "过", boneyard: "牌堆", yourTiles: "你的牌", left: "◀ 左", right: "右 ▶", pickEnd: "放在哪一端？", youWin: "你赢了！", cpuWins: "AI 获胜", blockDraw: "平局（封盘）", blocked: "封盘 — 剩余点数少者胜", thinking: "AI 思考中…", record: "战绩", level1: "学徒", level2: "行家", level3: "大师", start: "开始游戏", noMove: "没有可出的牌。" },
  fr: { title: "Dominos", you: "Vous", cpu: "IA", turn: "Tour", reset: "Nouvelle partie", draw: "Piocher", pass: "Passer", boneyard: "Pioche", yourTiles: "Vos dominos", left: "◀ Gauche", right: "Droite ▶", pickEnd: "Quel côté ?", youWin: "Vous gagnez !", cpuWins: "L'IA gagne", blockDraw: "Nul (bloqué)", blocked: "Bloqué — le moins de points gagne", thinking: "L'IA réfléchit…", record: "Bilan", level1: "Apprenti", level2: "Adepte", level3: "Maître", start: "Commencer", noMove: "Aucun domino jouable." },
  es: { title: "Dominó", you: "Tú", cpu: "IA", turn: "Turno", reset: "Nueva partida", draw: "Robar", pass: "Pasar", boneyard: "Pozo", yourTiles: "Tus fichas", left: "◀ Izq.", right: "Der. ▶", pickEnd: "¿Qué extremo?", youWin: "¡Has ganado!", cpuWins: "Gana la IA", blockDraw: "Empate (cerrado)", blocked: "Cerrado — gana quien menos puntos tenga", thinking: "La IA está pensando…", record: "Historial", level1: "Aprendiz", level2: "Experto", level3: "Maestro", start: "Empezar", noMove: "Sin ficha jugable." },
};
    ...
    <GameContainer title={t.title} subtitle={`${t.cpu}: ${cpu.length} · ${t.boneyard}: ${bone.length}`} onReset={newGame}>
      ...
      {pendingTile && (
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <button onClick={() => humanPlay(pendingTile, "left")} className="px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm font-bold">{t.left}</button>
          <button onClick={() => humanPlay(pendingTile, "right")} className="px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm font-bold">{t.right}</button>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {player.map((d) => {
            ...
            return (
              <button key={d.id} onClick={() => clickHandTile(d)} disabled={!playable}
                className={`bg-card border-2 rounded-lg shadow-sm flex flex-col divide-y divide-border transition-all ${
                  playable ? "border-primary hover:-translate-y-1 active:scale-95" : "border-border opacity-60"
        ...
        {turn === "you" && !winner && !humanHasMove && (
          <div className="flex flex-col items-center gap-1">
            <button onClick={humanDrawOrPass} className="px-5 py-2 rounded-lg bg-amber-500 text-white font-bold">
      ...
      {winner && (
        <div className="absolute inset-0 z-20 bg-background/80 backdrop-blur-md rounded-4xl flex flex-col items-center justify-center animate-in fade-in zoom-in-95">
          ...
          <button onClick={newGame} className="px-10 py-3 bg-primary text-primary-foreground rounded-full font-bold shadow-lg">{t.reset}</button>
        </div>
      )}
    </GameContainer>
  );
