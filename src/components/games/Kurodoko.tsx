...
export function generateKurodoko(difficulty: Difficulty): { puzzle: Puzzle; solution: Cell[][] } {
    ...
    whiteCells.sort(() => Math.random() - 0.5);
    let given = 0;
...
const COPY = {
    ko: { title: '쿠로도코', subtitle: 'Nikoli Logic', desc: '숫자 칸에서 보이는 흰 칸 수(자신 포함)가 숫자와 같아지도록 검은 칸을 칠하세요. 검은 칸은 붙을 수 없고 흰 칸은 모두 연결돼야 합니다.', easy: '쉬움 5×5', medium: '보통 6×6', hard: '어려움 7×7', win: '완벽한 추리력입니다!', errNumber: '숫자 조건이 깨졌습니다', errAdjacent: '검은 칸이 붙어 있습니다', errDisconnected: '흰 칸이 끊겼습니다', moves: '이동' },
    en: { title: 'Kurodoko', subtitle: 'Nikoli Logic', desc: 'Shade black cells so each number sees exactly that many white cells (itself included). Black cells never touch; whites stay connected.', easy: 'Easy 5×5', medium: 'Medium 6×6', hard: 'Hard 7×7', win: 'Flawless deduction!', errNumber: 'A number clue is broken', errAdjacent: 'Black cells are touching', errDisconnected: 'White cells are disconnected', moves: 'Moves' },
    ja: { title: 'クロドコ（黒どこ）', subtitle: 'Nikoli Logic', desc: '数字マスから見える白マスの数(自分含む)が数字と一致するよう黒マスを塗ります。黒マスは隣接不可、白マスは全て連結。', easy: 'かんたん 5×5', medium: 'ふつう 6×6', hard: 'むずかしい 7×7', win: '完璧な推理力です！', errNumber: '数字の条件が崩れています', errAdjacent: '黒マスが隣接しています', errDisconnected: '白マスが分断されています', moves: '手数' },
    zh: { title: '黑格在哪 (Kurodoko)', subtitle: 'Nikoli Logic', desc: '涂黑格子，使每个数字格能看到的白格数(含自身)恰好等于该数字。黑格不相邻，白格须全连通。', easy: '简单 5×5', medium: '普通 6×6', hard: '困难 7×7', win: '推理完美！', errNumber: '数字条件被破坏', errAdjacent: '黑格相邻了', errDisconnected: '白格断开了', moves: '步数' },
    fr: { title: 'Kurodoko', subtitle: 'Nikoli Logic', desc: 'Noircissez des cases pour que chaque nombre voie exactement autant de cases blanches (lui compris). Les noires ne se touchent pas ; les blanches restent connectées.', easy: 'Facile 5×5', medium: 'Moyen 6×6', hard: 'Difficile 7×7', win: 'Déduction parfaite !', errNumber: 'Un indice numérique est cassé', errAdjacent: 'Des cases noires se touchent', errDisconnected: 'Les cases blanches sont coupées', moves: 'Coups' },
    es: { title: 'Kurodoko', subtitle: 'Nikoli Logic', desc: 'Sombrea casillas para que cada número vea exactamente esa cantidad de blancas (incluida ella). Las negras no se tocan; las blancas quedan conectadas.', easy: 'Fácil 5×5', medium: 'Normal 6×6', hard: 'Difícil 7×7', win: '¡Deducción perfecta!', errNumber: 'Una pista numérica está rota', errAdjacent: 'Hay negras juntas', errDisconnected: 'Las blancas están separadas', moves: 'Movs' },
} as const;
...
const Kurodoko: React.FC<{ locale?: string }> = ({ locale = 'ko' }) => {
    ...
    const [puzzle, setPuzzle] = useState<Puzzle>(() => generateKurodoko('easy').puzzle);
    ...
    const newPuzzle = useCallback((diff: Difficulty) => {
        const { puzzle: p } = generateKurodoko(diff);
        setDifficulty(diff);
        setPuzzle(p);
    ...
    const toggle = (r: number, c: number) => {
        ...
        setBoard(next);
        setMoves((m) => m + 1);
        setValidation(validateKurodoko(next, puzzle));
    };
    ...
    return (
        <GameContainer title={t.title} subtitle={t.subtitle} onReset={() => newPuzzle(difficulty)}>
