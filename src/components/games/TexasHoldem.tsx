...
const COPY: Record<UILocale, {
  ...
  ko: {
    ...
    note: "실제 화폐·베팅 없는 학습용 게임입니다. 두 장의 핸드와 커뮤니티 5장으로 가장 강한 5장을 만듭니다.",
  },
  en: {
    ...
    note: "A learning game with no real money or betting. Make the best 5-card hand from your 2 cards plus 5 community cards.",
  },
  ja: {
    ...
    note: "実際のお金やベットのない学習用ゲームです。手札2枚とコミュニティ5枚で最強の5枚を作ります。",
  },
  zh: {
    ...
    note: "这是一款无真实货币和下注的学习游戏。用你的2张牌加5张公共牌组成最强的5张牌。",
  },
  fr: {
    ...
    note: "Jeu d'apprentissage sans argent réel ni mise. Composez la meilleure main de 5 cartes avec vos 2 cartes et les 5 cartes communes.",
  },
  es: {
    ...
    note: "Un juego de aprendizaje sin dinero real ni apuestas. Forma la mejor mano de 5 cartas con tus 2 cartas y las 5 comunitarias.",
  },
};
        ...
        <div className="flex flex-col gap-5">
          ...
          {over ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <button onClick={deal} className="px-6 py-2.5 rounded-xl bg-primary text-primary-foreground font-black hover:bg-primary/90 transition-colors">{t.again}</button>
            </div>
          ) : (
            <div className="flex justify-center gap-3">
              <button onClick={fold} className="px-5 py-2.5 rounded-xl border border-border font-bold text-sm hover:bg-muted transition-colors">{t.fold}</button>
              <button onClick={advance} className="px-6 py-2.5 rounded-xl bg-primary text-primary-foreground font-black hover:bg-primary/90 transition-colors">
          ...
          <div className="border-t border-border pt-3">
            <button onClick={() => setShowRanks((v) => !v)} className="w-full flex items-center justify-between text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
