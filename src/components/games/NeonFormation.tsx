...
import { reviewNeonSortie } from "../../lib/games/neon-formation";
  ...
  return <section className="not-prose relative left-1/2 my-8 w-[min(100vw-1rem,1180px)] -translate-x-1/2 overflow-hidden rounded-3xl border border-slate-200 bg-white text-slate-950 shadow-xl">
    ...
    <div className="relative grid gap-10 p-6 sm:p-10 lg:grid-cols-[1.1fr_.9fr] lg:p-14">
      <div><p className="font-mono text-[11px] font-black tracking-[.25em] text-cyan-700">{t.eyebrow}</p><h2 className="mt-4 text-5xl font-black uppercase leading-[.86] tracking-[-.055em] sm:text-7xl">{t.title}</h2><p className="mt-6 max-w-xl text-sm leading-7 text-slate-600 sm:text-base">{t.subtitle}</p>
        ...
        {result && <><dl role="status" aria-live="polite" className="mt-7 grid grid-cols-4 gap-2 rounded-2xl border border-slate-200 bg-white/60 p-4 text-center">{[[t.score, result.score], [t.wave, result.wave], [t.accuracy, `${result.accuracy}%`], [t.rescued, result.rescues]].map(([label, value]) => <div key={label}><dt className="text-[9px] font-bold uppercase text-slate-400">{label}</dt><dd className="mt-1 font-mono text-lg font-black">{value}</dd></div>)}</dl><p className="mt-2 text-center text-xs font-black text-cyan-700">→ {reviewNeonSortie(result) === "accuracy" ? t.accuracy : reviewNeonSortie(result) === "rescue" ? t.rescued : t.wave}</p></>}
      </div>
