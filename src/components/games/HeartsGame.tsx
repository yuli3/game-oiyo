...
const HeartsGame: React.FC<{ locale?: string }> = ({ locale = "ko" }) => {
  ...
  const reset = useCallback(() => { setGame(createHeartsGame()); setPassSelection([]); }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setGame((current) => {
      ...
      const choice = chooseHeartsCpuCard(current, current.currentPlayer);
      return playHeartsCard(current, current.currentPlayer, choice.id);
    }), 360);
  ...
  const play = (card: HeartsCard) => { if (legalIds.has(card.id)) setGame((current) => playHeartsCard(current, 0, card.id)); };
      ...
      {(game.phase === "passing" || game.phase === "playing") && (
        <section aria-label={`${t.cardsLeft}: ${game.hands[0].length}`}>
          ...
          <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2">{game.hands[0].map((card) => {
            ...
            const enabled = game.phase === "passing" ? selected || passSelection.length < 3 : legal;
