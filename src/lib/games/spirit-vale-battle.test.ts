...
describe("damage", () => {
  it("hits harder into the phase it overcomes than into one that overcomes it", () => {
    // 木剋土 both ways: wood→earth is strong, metal→wood is strong against wood.
    const strong = computeDamage(wood, earth, fixed(0.5));
    const weak = computeDamage(wood, metal, fixed(0.5));
    expect(strong).toBeGreaterThan(weak);
  });

  it("is reduced when feeding the defender", () => {
    // 木生火 — attacking fire with wood feeds it.
    const feeding = computeDamage(wood, fire, fixed(0.5));
    const neutral = computeDamage(wood, wood, fixed(0.5));
  ...
  it("never deals less than one damage even in the worst matchup", () => {
    for (const attacker of SPIRITS) {
      for (const defender of SPIRITS) {
        expect(computeDamage(attacker, defender, fixed(0))).toBeGreaterThanOrEqual(1);
      }
    }
  });

  it("varies with the roll but stays bounded", () => {
    const low = computeDamage(wood, earth, fixed(0));
    const high = computeDamage(wood, earth, fixed(0.999));
...
describe("battle flow", () => {
  ...
  it("does not mutate the state it is given", () => {
    ...
    resolveTurn(s, { type: "attack" }, fixed(0.5));
  ...
  it("ignores further actions once resolved", () => {
    ...
    const again = resolveTurn(fled, { type: "attack" }, fixed(0.5));
  ...
  it("lets the faster spirit strike first", () => {
    ...
    const s = resolveTurn(createBattle(fast, slow), { type: "attack" }, fixed(0.5));
  ...
  it("resolves to a win when the wild spirit faints", () => {
    ...
    s = resolveTurn(s, { type: "attack" }, fixed(0.5));
  ...
  it("costs the turn when a capture fails", () => {
    ...
    // The wild spirit got its free swing.
    expect(s.log.some((e) => e.kind === "attack" && e.by === "wild")).toBe(true);
...
describe("bare-handed encounters", () => {
  ...
  it("passes the turn instead of crashing when attacking with no party", () => {
    const s = resolveTurn(createBattle(earth, null), { type: "attack" }, fixed(0.5));
    expect(s.phase).toBe("active");
    expect(s.turn).toBe(2);
  ...
  it("cannot be lost when there is nothing to lose", () => {
    ...
    for (let i = 0; i < MAX_TURNS + 5 && s.phase === "active"; i++) {
      s = resolveTurn(s, { type: "attack" }, rng);
    }
...
describe("termination", () => {
  ...
  it("reports exhaustion rather than hanging when neither side can finish", () => {
    ...
    while (s.phase === "active") s = resolveTurn(s, { type: "attack" }, rng);
