...
export const GARDEN_FIELD_RADIUS=170;
...
export function plantGardenSeed(state:GravityGardenState,type:GardenSeedType,x:number,y:number):GravityGardenState{
 if(!Number.isFinite(x)||!Number.isFinite(y)||state.time>0||state.running||state.seeds.length>=6||x<24||y<24||x>GRAVITY_GARDEN_WIDTH-24||y>GRAVITY_GARDEN_HEIGHT-24)return state;
 return {...state,seeds:[...state.seeds,{id:state.seeds.length,type,x:Math.round(x),y:Math.round(y)}]};
}

export function startGravityGarden(state:GravityGardenState):GravityGardenState{return state.running||state.time>0||state.seeds.length===0?state:{...state,running:true};}

function field(drop:GardenDrop,seeds:readonly GardenSeed[]){let ax=12,ay=22,damping=1;for(const seed of seeds){const dx=seed.x-drop.x,dy=seed.y-drop.y,d2=dx*dx+dy*dy;if(d2>GARDEN_FIELD_RADIUS*GARDEN_FIELD_RADIUS)continue;const distance=Math.max(24,Math.sqrt(d2)),falloff=1-distance/GARDEN_FIELD_RADIUS;if(seed.type==="gravity-bloom"){ax+=dx/distance*92*falloff;ay+=dy/distance*92*falloff;}else if(seed.type==="wind-reed"){ax+=105*falloff;ay-=20*falloff;}else damping*=1-0.75*falloff;}return {ax,ay,damping};}
...
export function stepGravityGarden(state:GravityGardenState,dt=GRAVITY_GARDEN_STEP):GravityGardenState{
 if(!state.running||!Number.isFinite(dt)||dt<=0||dt>GRAVITY_GARDEN_STEP)return state;const time=Math.min(GRAVITY_GARDEN_DURATION,state.time+dt);
