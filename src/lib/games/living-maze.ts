export type MazePoint = Readonly<{ x: number; y: number }>;
export type MazeGate = Readonly<{ x: number; y: number; need: 'light' | 'quiet'; open: boolean; known: boolean; patience: number }>;
export type MazeState = Readonly<{ seed: number; player: MazePoint; gates: readonly MazeGate[]; light: number; stress: number; turns: number; phase: 'exploring' | 'escaped' | 'overwhelmed' | 'expired'; event: 'start' | 'moved' | 'blocked' | 'observed' | 'alone' | 'opened' | 'waiting' | 'refused' | 'empty' | 'noise'; }>;
export type MazeAction = 'up' | 'down' | 'left' | 'right' | 'observe' | 'offer' | 'wait' | 'call';
export const MAZE_TURN_LIMIT = 40;
export function createLivingMaze(seed = 0): MazeState {
...
export function mazeTile(s: MazeState, x: number, y: number): 'wall' | 'gate' | 'floor' | 'exit' {
...
export function nearbyGate(s: MazeState): number {
  return s.gates.findIndex(g => !g.open && Math.abs(s.player.x - g.x) + Math.abs(s.player.y - g.y) === 1);
}
export function advanceLivingMaze(s: MazeState, action: MazeAction): MazeState {
  ...
  let player = s.player, light = s.light, stress = s.stress, event: MazeState['event'] = 'waiting';
  ...
  if (action === 'offer') {
    ...
    gate.known = true;
    if (gate.need === 'light') {gate.open = true; event = 'opened';}
    else {stress += 2; event = 'refused';}
  } else if (action === 'wait') {
    ...
    stress += 3; event = 'noise';
    // Do not close a living wall onto the player standing in its doorway.
